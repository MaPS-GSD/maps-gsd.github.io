 ...
 * Uses a pixel-color based approach better than v1.
 * Still alpha blending is not perfect, there is a better version v3 (without alpha).
 */
onmessage = (event) => {
  ...
  // Create chroma scale
  const colorScale = chroma.scale(data.COLOR_SCALE_NAME)
    .mode(data.COLOR_SCALE_INTERPOLATION_MODE);
  const colorScaleLen = data.COLOR_SCALE_RANGE[1] - data.COLOR_SCALE_RANGE[0];
  ...
  for (let i = 0; i < gazeSets.length; i++) {
    ...
    for (let j = 0; j < gazeSet.length; j++) {
      ...
      // Compute base color for this gaze point 
      n = total / gazeCount;
      nc = data.COLOR_SCALE_RANGE[0] + colorScaleLen * n;
      rgb = colorScale(nc).rgb();
